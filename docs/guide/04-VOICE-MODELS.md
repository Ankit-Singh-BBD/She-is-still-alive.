...
The repository defaults name `gemini-3.5-flash-lite` for reasoning and `gemini-3.1-flash-live-preview` for live voice. They are source-code strings. Their current account availability, free quotas, billing behavior and quality were NOT independently checked for this documentation rewrite. Comments claiming they answered in another session are not today's probe.
...
## Free plus no quality loss: the actual constraint
...
- **Local-only:** no paid inference API; works without internet for local capabilities. Quality and speed depend on hardware/model. Web research is unavailable offline.
- **Free-tier hybrid:** local baseline plus explicitly allowed hosted quotas. When quota ends, queue, block or use an already-qualified local route and label limitations. Never switch to paid billing automatically.
- **Quality-priority:** evaluate stronger options; enable paid usage only after an owner budget decision. This mode does not satisfy a strict zero-paid requirement by default.

For this owner the default is zero-paid. If no zero-paid candidate passes a required quality gate, that capability is BLOCKED, not silently accepted with lower quality. The owner must choose more local resources, a narrower capability, waiting for quota, or a different budget. Do not rotate accounts to evade quotas.
